...
- 代扩展执行文件创建和外部 App 启动，并在失败时向用户呈现错误。文件创建先在主线程
  固定配置与剪贴板文本，再在后台读取模板、编码和写盘，完成后回主线程更新状态与选中文件
...
- 菜单文本字段在 400ms 无新输入后合并落盘；Toggle、增删、排序和终端切换立即
  落盘。保存失败保留未保存状态，设置页提供重试入口；关闭设置或退出时也会重试。
  设置页关闭与 App 退出会同步冲刷，初始化发现终端能力缺失也会静默补写
...
- CLI、终端与配置化 CLI 的异步执行生命周期统一由 `DeepLinkCoordinator.perform`
  管理，三条路径共享开始、成功、失败记录与可信错误呈现语义。终端启动按接收顺序
  串行执行（包括不同终端，避免全局焦点与快捷键竞争），单次失败不阻断后续请求
