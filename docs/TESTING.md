...
- 首次向导第一步点击“稍后再说”，确认主窗口仍可使用；再从设置的诊断栏重新运行
  向导，确认三步流程完整且原有菜单、终端配置未被清除
...
- 冷启动主窗口时确认诊断刷新期间显示进度，完成后的通过/注意数量与设置诊断栏一致
- 分别从主窗口、设置的监控目录和菜单栏点击“重启 Finder”，确认三处都先说明会
  关闭所有 Finder 窗口；取消后 Finder 不退出，确认后才重启
